import { buildPropertyPhotoMap, withLegacyPropertyCover } from "@/entities/property/api/photo-utils";
...
import type { SupabasePropertyPhotoRow, SupabasePropertyRow } from "@/shared/api/supabase/types";
...
export async function getOwnerProperties(): Promise<OwnerPropertyListItem[]> {
  ...
  const photoMap = buildPropertyPhotoMap((photoRows ?? []) as SupabasePropertyPhotoRow[]);
