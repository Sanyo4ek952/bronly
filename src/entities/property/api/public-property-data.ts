...
function toPublicFallbackData(filters: PublicStayFilters): PublicPropertyPageData {
  return {
    ...
    rooms: mockRooms
      ...
      .sort((a, b) => Number(b.isAvailableForFilter) - Number(a.isAvailableForFilter)),
    filters,
...
export const getPublicPropertyPageData = cache(async (
  slug: string,
  filterInput: {
    checkIn?: string;
    checkOut?: string;
  ...
  try {
    ...
    const [{ data: roomRows }, { data: featureRows }, { data: ruleRows }] = await Promise.all([
      supabase
        .from("rooms")
        .select("*")
