...
const MAX_ROOM_PHOTOS_PER_UPLOAD = 10;
...
function buildStoragePath(profileId: string, roomId: string, fileName: string) {
  return `${profileId}/rooms/${roomId}/${Date.now()}-${crypto.randomUUID()}-${sanitizeFileName(fileName)}`;
}
...
export function getUploadedRoomPhotoFiles(formData: FormData) {
  return getUploadedFiles(formData, "photos");
}

export function validateRoomPhotoFiles(files: File[]) {
  if (files.length > MAX_ROOM_PHOTOS_PER_UPLOAD) {
    return "room-photo-count";
  }
  ...
  for (const file of files) {
    ...
    if (fileError) {
      return fileError === "photo-type" ? "room-photo-type" : "room-photo-size";
    }
  }
...
export async function uploadRoomPhotoFiles(
  ...
  for (const file of files) {
    const storagePath = buildStoragePath(profileId, roomId, file.name);
