...
Снимок задачи 01 от 2026-09-14, исходный commit: `5773c48`. Продуктовая опора: PRD, разделы 12 (стек), 14 (RLS), 17 (готовность MVP). Это инструкция для существующего checkout.
...
Тесты фиксируют существующие правила сезонных границ (последняя дата сезона включена), тарифные пороги 3/10 и трёхдневный grace period; это техническая защита текущего поведения, а не новые продуктовые решения. Согласование календарного UI с интервалами проживания остаётся задачей 04. Подсчёт номеров через БД, RLS, уведомления и сохранение заявки unit-тестами не подтверждаются.
